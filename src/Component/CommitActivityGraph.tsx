...
const CommitActivityGraph: React.FC<CommitActivityGraphProps> = ({
  owner,
  repoName,
}) => {
  ...
  const [contributorsChanges, setContributorsChanges] = useState<
    ContributorChange[]
  >([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const [totalChangesResponse, contributorsChangesResponse] =
          await Promise.all([
  ...
  if (totalChanges && totalChanges.length > 0) {
    selectedData =
      selectedOption === "commits"
        ? totalChanges.map((week) => week[0])
        : selectedOption === "additions"
        ? totalChanges.map((week) => week[1])
  ...
  const contributorsChangesOptions = {
    ...
    series: Array.isArray(contributorsChanges)
      ? contributorsChanges.map((contributor) => ({
    ...
    <div className="container mx-auto px-4">
      <div className="mb-4 md:ml-20">
        ...
        <select
          ...
          className="ml-2 border border-2 bg-[#D5F3FD99] rounded "
        >
      ...
      <div>
        {loading ? (
          ...
          contributorsChanges.length > 0 && (
            <HighchartsReact
              highcharts={Highcharts}
              options={contributorsChangesOptions}
            />
          )
        )}
      </div>
